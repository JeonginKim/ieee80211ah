...
{
//	printf("[JI_LOG] EdcaTxopN::NotifyAccessGranted is called ");
//	std::cout << this << std::endl;

  NS_LOG_FUNCTION (this);
  if (!AccessIfRaw)
    {
//	    printf("[JI_LOG] EdcaTxopN::AccessIfRaw is not true! ");
//		std::cout << this << std::endl;
        return;
    }
//  printf("[JI_LOG] EdcaTxopN::AccessIfRaw is true!!!!!!!!!!!!!!!!!!! ");
//  std::cout << this << std::endl;
    int newdata=99;
...
//		  printf("[JI_LOG] EdcaTxopN::queue is empty\n");
          return;
        }
...
//	  		  printf("[JI_LOG] EdcaTxopN::no available packets in the queue\n");
              return;
            }
...
EdcaTxopN::MissedCts (void)
{
  ...
  if (!NeedRtsRetransmission ())
    ...
    {
	  Mac48Address addr = m_currentHdr.GetAddr2 ();
	   uint8_t buff [6] = {0,};
	   addr.CopyTo(buff);
	   uint8_t node = (uint8_t)(buff[5]-1);

      m_dcf->UpdateFailedCw (node);
    }
  m_dcf->StartBackoffNow (m_rng->GetNext (0, m_dcf->GetCw ()));
  RestartAccessIfNeeded ();
}
...
//	   std::cout << "[JI_LOG] " << m_currentHdr.GetAddr2 () << " ";
...
{
//	printf("[JI_LOG] EdcaTxopN::AccessAllowedIfRaw is called : %d\n", allowed);
...
EdcaTxopN::RestartAccessIfNeeded (void)
{
  ...
  if ((m_currentPacket != 0
      ...
      && AccessIfRaw)
    {
//      printf("[JI_LOG] EdcaTxopN::RestartAccessIfNeeded is called ");
//	  std::cout << this << std::endl;
...
EdcaTxopN::StartAccessIfNeeded (void)
{
  ...
  if (m_currentPacket == 0
    ...
    {
//         printf("[JI_LOG] EdcaTxopN::StartAccessIfNeeded is called ");
//		 std::cout << this << std::endl;
...
EdcaTxopN::StartAccessIfNeededRaw (void)
{
    ...
    if ((!m_queue->IsEmpty () || m_baManager->HasPackets ())
        ...
        && AccessIfRaw)    // always TRUE outside RAW
    {
//          printf("[JI_LOG] EdcaTxopN::StartAccessIfNeededRaw is called ");
//		  std::cout << this << std::endl;

        m_manager->RequestAccess (m_dcf);
    }

//	printf("[JI_LOG] EdcaTxopN::StartAccessIfNeededRaw is called %d %d %d ", m_queue->IsEmpty (), m_baManager->HasPackets (), AccessIfRaw);
//	std::cout << this << std::endl;
}
...
{
//	printf("[JI_LOG] EdcaTxopN::RawStart is called ");
//	std::cout << this << std::endl;
...
{
//	printf("[JI] EdcaTxopN::OutsideRawStart is called\n");

  NS_LOG_FUNCTION (this);
//    printf("[JI_LOG] EdcaTxopN::OutsideRawStart is called true! ");
//	std::cout << this << std::endl;
...
{
//	printf("[JI_LOG] EdcaTxopN::StartNext is called\n");
...
{
//	printf("[JI_LOG] EdcaTxopN::SendBlockAckRequest is called\n");
...
{
//	printf("[JI_LOG] EdcaTxopN::SendAddBaRequest is called\n");
