...
//		printf("[JI_LOG] destAID : %d\n", destAID);
		...
		if(destAID < 250)
		{
			...
			printf("[JI_LOG] Received Node ID : %d Access Time :\t%lf\t DiffTime \t%d\tcw : \t%d\tfreeze : \t%d \tSlot Num : %d\t\n", destAID + 1, Simulator::Now ().GetSeconds(), diff,  g_cw[destAID], g_freeze[destAID], g_statRawSlot[destAID]);
