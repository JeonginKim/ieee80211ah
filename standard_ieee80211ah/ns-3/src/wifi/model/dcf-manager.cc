...
DcfState::OutsideRawStart (void)
{
 ...
 //std::cout << "[JI_LOG] OutsideRawStart is called " << "m_cw=" << m_cw_temp << "m_backoffslots=" << m_backoffSlots_temp <<std::endl;
...
void
DcfState::UpdateFailedCw (uint32_t node)
{
  //see 802.11-2012, section 9.19.2.5
//  m_cw = std::min ( (m_cw + 1) - 1, m_cwMax); 
    m_cw = std::min ( 2 * (m_cw + 1) - 1, m_cwMax); 
  //g_cw[node] = m_cw;
//  printf("[JI_LOG] UpdataFailedCw is Called Node : %d Access time : ", node+1);
...
//  std::cout << "[JI_LOG] " << "update slots=" << nSlots << " slots, backoff=" << m_backoffSlots;
}
...
//  printf("[JI_LOG] StartBackoffNow CW: %d CW Min : %d CW Max : %d CW Range : %d \n",  nSlots, m_cwMin, m_cwMax, m_cw);
...
{
//	printf("[JI_LOG] DcfState::NotifyAccessGranted is called ");
...
{
//	printf("[JI_LOG] DcfManager::RequestAccess is called..\n");
...
{
//	printf("[JI_LOG] DcfManager::DoGrantAccess is called ");
//	std::cout << this << std::endl;
  ...
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); k++)
    {
      ...
      if (state->IsAccessRequested ()
          && GetBackoffEndFor (state) <= Simulator::Now () )
        {
           ...
           */
//           printf("[JI] DcfManager::DoGrantAccess is called\n");
...
{
//	printf("[JI_LOG] DcfManager::AccessTimeout is called..\n");

  NS_LOG_FUNCTION (this);
  UpdateBackoff ();
//   printf("[JI] DcfManager::AccessTimeout is called\n");
  DoGrantAccess ();
  DoRestartAccessTimeoutIfNeeded ();
}
...
void DcfManager::SetID(uint32_t node)
{
	m_node = node;
}
...
DcfManager::NotifyMaybeCcaBusyStartNow (Time duration)
{
	...
	{
		g_freeze[m_node]++;
//		printf("[JI_LOG] freeze increased node : %d count : %d\n", m_node, g_freeze[m_node]);
	}
