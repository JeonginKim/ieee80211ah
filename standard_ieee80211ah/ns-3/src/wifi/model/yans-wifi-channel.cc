...
//		 m_nowTime = clock();
//		  printf("[JI] YansWifiChannel::Send is called srcNode : %d dstNode : %d Packet Size : %d Time : %f(sec)  DiffTime : %f(sec)\n", 
//		  	srcNode, dstNode, packet->GetSize(), (float)(m_nowTime) / (float)CLOCKS_PER_SEC, (float)(m_nowTime - m_prevTime) / (float)CLOCKS_PER_SEC);
