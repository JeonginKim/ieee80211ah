...
{
//	printf("[JI_LOG] CheckAssoc is called \n");
//	std::cout << "[JI_LOG] simulTime =" << simulationTime << std::endl;
...
//		printf("[JI_LOG] GetAssocNum () == Nsta is called %d %d randomInterval %f\n", GetAssocNum (), Nsta, randomInterval);
		...
		else if(Simulator::Now ().GetSeconds () < 120)
		{
			payloadLength	= 34;
		}
		else if(Simulator::Now ().GetSeconds () < 180)
		{
			payloadLength	= 39;
		}
		else if(Simulator::Now ().GetSeconds () < 240)
		{
			payloadLength	= 44;
		}
		else if(Simulator::Now ().GetSeconds () < 300)
		{
			payloadLength	= 49;
		}
		else if(Simulator::Now ().GetSeconds () < 360)
		{
			payloadLength	= 54;
		}
		else if(Simulator::Now ().GetSeconds () < 420)
		{
			payloadLength	= 59;
		}
		else if(Simulator::Now ().GetSeconds () < 480)
		{
			payloadLength	= 14;
		}
		else if(Simulator::Now ().GetSeconds () < 540)
		{
			payloadLength	= 14;
		}
		else
...
/*
		...
		else if(Simulator::Now ().GetSeconds () < 60)
		{
			payloadLength	= 44;
		}
		else if(Simulator::Now ().GetSeconds () < 80)
		{
			payloadLength	= 49;
		}
		else if(Simulator::Now ().GetSeconds () < 100)
		{
			payloadLength	= 54;
		}
		else if(Simulator::Now ().GetSeconds () < 120)
		...
		else if(Simulator::Now ().GetSeconds () < 140)
		{
			payloadLength	= 54;
		}
		else if(Simulator::Now ().GetSeconds () < 160)
		{
			payloadLength	= 49;
		}
		else if(Simulator::Now ().GetSeconds () < 180)
...
/*
		...
		else if(Simulator::Now ().GetSeconds () < 12)
		{
			payloadLength	= 59;
		}
		else if(Simulator::Now ().GetSeconds () < 14)
		{
			payloadLength	= 54;
		}
		else if(Simulator::Now ().GetSeconds () < 16)
		{
			payloadLength	= 49;
		}
...
*/
		...
		g_bytePerNode = payloadLength + 66;
//		g_traffic == 0;
...
//            std::cout << "Generate traffic in " << randomStart << "seconds, station " << double(i) << std::endl;
            ...
            clientApp.Start (Seconds (randomStart));
            //clientApp.Stop (Seconds (simulationTime));
            clientApp.Stop (Seconds (randomInterval));
          }
        AppStartTime=Simulator::Now ().GetSeconds (); //clientApp start time

		if(bFirst)
		{
			bFirst = false;
			Simulator::Schedule(Seconds (simulationTime), &GetQueuelen, Nsta); //check before simulation stop
	        Simulator::Stop (Seconds (simulationTime)); //set stop time until no packet in queue
		}
        Simulator::Schedule(Seconds(randomInterval), &CheckAssoc, Nsta, simulationTime, wifiApNode, wifiStaNode, apNodeInterface);
      }
    else
      {
        printf("[JI_LOG] else is called %d %d\n", GetAssocNum (), Nsta);
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("NRawSta", "number of stations supporting RAW", NRawSta);	// No Station Per RAW group
  cmd.AddValue ("SlotFormat", "format of NRawSlotCount", SlotFormat);
  cmd.AddValue ("NRawSlotCount", "SlotDurationCount", NRawSlotCount);
  cmd.AddValue ("NRawSlotNum", "Slot per RAW group", NRawSlotNum);
...
// 	std::cout << "[JI]=======NGroupStas : " << NGroupStas << " NRawSlotNum : " << NRawSlotNum << std::endl;
...
//	std::cout << "[JI]===========Make Random Generate Traffic===============" << std::endl;
    Simulator::Schedule(Seconds(1), &CheckAssoc, Nsta, simulationTime, wifiApNode, wifiStaNode,apNodeInterface);


    Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
//	std::cout << "[JI]===========Make ARPCache===============" << std::endl;
    PopulateArpCache ();
    
      if  (OutputPosition)
        {
//       	std::cout << "[JI]===========Make Mobility===============" << std::endl;
          int i =0;
          while (i < Nsta)
            {
