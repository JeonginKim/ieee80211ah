...
namespace ns3 {
	extern int g_dataPacketCount;
	extern int g_allPacketCount;
	extern int g_ap_receivePacketCount;

	extern Time g_prevTime[300];
	extern Time g_prevTime_ack[300];
	extern int g_cw[300];
	extern int g_freeze[300];

	extern int g_contentionCount;
	extern int g_receviedCount;
	extern int g_statRawSlot[300];
	extern Time g_nowBeaconTime;

	extern uint64_t g_delayTotal;
	extern uint64_t g_receivedPacketTotal;
	extern uint64_t g_maxDelay;
	extern uint64_t g_throughput;

	//extern std::queue<Time> g_sendTime[3000];
	extern Time g_sendTime[300];

	extern bool g_bIsInsideBackoff[300];

	//new feature
	extern bool g_bIsSent[300];
	extern uint8_t  g_noSentCount[300];

	extern uint8_t  g_maxCount;
	extern uint8_t  g_threshold;
	extern uint8_t  g_prevTraffic;
	extern uint8_t	g_traffic;
	extern uint8_t	g_bytePerNode;
