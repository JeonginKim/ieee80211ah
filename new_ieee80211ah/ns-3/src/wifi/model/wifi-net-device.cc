...
//  printf("[JI_LOG] WifiNetDevice::Send is called %d\n", packet->GetSize());

    // JI Added
  uint32_t srcNode = GetNode()->GetId();
  int packetSize = packet->GetSize();

//if(packetSize == 322 && Simulator::Now().GetSeconds() > 10 && Simulator::Now().GetSeconds() < 110)

if(packetSize == 50 || packetSize == 70 || packetSize == 75 || packetSize == 80 || packetSize == 85 || packetSize == 90 || packetSize == 95)
//if(packetSize == 70 || packetSize == 75 || packetSize == 80 || packetSize == 85 || packetSize == 90 || packetSize == 95)
//if(packetSize == 70 || packetSize == 75 || packetSize == 80 || packetSize == 102 || packetSize == 103)
{
//	if(g_bIsInsideBackoff[srcNode] == true)
//		return true;

	g_dataPacketCount++;

	if(g_prevTime[srcNode] == 0)
	{
		g_prevTime[srcNode] = Simulator::Now ();
		g_prevTime_ack[srcNode] = Simulator::Now ();
	}

	printf("At %f NodeID %d Accesses %d DiffTime %I64d\n", Simulator::Now ().GetSeconds(), srcNode+1, g_dataPacketCount, Simulator::Now ().GetMicroSeconds() - g_prevTime[srcNode].GetMicroSeconds());
//	printf("[JI_LOG] Send Node ID : %d Access time : ", srcNode+1);
//	std::cout << Simulator::Now().GetSeconds() << " Slot Num : " << g_statRawSlot[srcNode] << std::endl;
	g_contentionCount++;

	g_prevTime[srcNode] = Simulator::Now ();

	//g_sendTime[srcNode].push(Simulator::Now());
	g_sendTime[srcNode] = Simulator::Now();

	g_bIsSent[srcNode] = true;
	g_noSentCount[srcNode] = 0;

}
else
{
//	printf("[JI] No Data Packet YansWifiPhy::SendPacket is called srcNode : %d Packet Size : %d ControlPacketCount : %d\n", srcNode, packetSize, g_allPacketCount);
	g_allPacketCount++;
}
...
{
//	std::cout << "[JI_LOG] Received Packet Size : " << packet->GetSize() << " from : " << from << " to : " << to << std::endl;
	int packetSize = packet->GetSize();
	if(packetSize == 50 || packetSize == 70 || packetSize == 75 || packetSize == 80 || packetSize == 85 || packetSize == 90 || packetSize == 95)
	//if(packetSize == 70 || packetSize == 75 || packetSize == 80 || packetSize == 85 || packetSize == 90 || packetSize == 95)
	//if(packetSize == 85 || packetSize == 90 || packetSize == 95 || packetSize == 100 || packetSize == 105)
	//if(packetSize == 70 || packetSize == 75 || packetSize == 80 || packetSize == 102 || packetSize == 103)
	{
		uint8_t buff [6] = {0,};
		from.CopyTo(buff);
		uint16_t destAID = (uint16_t)(buff[4] * 256);
		destAID += (uint16_t)(buff[5]-1);
		// 00:00:00:00:00:00 : broadcast, 00:00:00:00:00:01 ~ 00:00:00:00:03:e8(1000), 00:00:00:00:03:e9 = 1001(AP)
//		printf("[JI_LOG] destAID : %d\n", destAID);

		if(destAID < 250)
		{
			g_receivedPacketTotal++;

			//int64_t diff = Simulator::Now ().GetMicroSeconds()- g_sendTime[destAID].front().GetMicroSeconds();
			int64_t diff = Simulator::Now ().GetMicroSeconds()- g_sendTime[destAID].GetMicroSeconds();
			//g_sendTime[destAID].pop();

			if(g_bIsSent[destAID] == false)
				diff = 0;
			...
			printf("At %f receivedACK DiffTime %I64d\n", Simulator::Now ().GetSeconds(), diff);
//			printf("[JI_LOG] Received Node ID : %d Access Time :\t%lf\t DiffTime \t%d\tcw : \t%d\tfreeze : \t%d \tSlot Num : %d\t noSentCount : %d\n", destAID + 1, Simulator::Now ().GetSeconds(), diff,  g_cw[destAID], g_freeze[destAID], g_statRawSlot[destAID], g_noSentCount[destAID]);

			g_prevTime_ack[destAID] = Simulator::Now ();
			g_freeze[destAID] = 0;
			g_receviedCount++;

			g_bIsSent[destAID] = false;
			g_noSentCount[destAID] = 0;
