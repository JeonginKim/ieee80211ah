...
void
RPS::RawAssignment::SetReservedInfo (uint8_t reserved)
{
  m_reserved = reserved;
}
...
uint8_t
RPS::RawAssignment::GetReservedInfo (void) const
{
  return m_reserved;
}
...
//suppose all subfield of RAW Assignment are presented, 13 octets
// change in future
void
RPS::SetRawAssignment (RPS::RawAssignment raw)
{
    m_length = 0;
    assignment = raw;
    static uint8_t m_rpsarry[13]; //! Support up to 10 RAW Assignment subfield
    ...
    m_length++;
	m_rpsarry[m_length] = (uint8_t)assignment.GetReservedInfo ();
    m_length++;
    m_rps = m_rpsarry;
