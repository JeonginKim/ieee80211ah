...
{
//	std::cout << "[JI]====================ApWifiMac::GetTypeId is called================" << std::endl;
...
{
//	std::cout << "[JI]====================SetAddress is called================" << address << std::endl;
...
{
//	std::cout << "[JI]====================SetBeaconGeneration is called================" << enable << std::endl;
  ...
  if (!enable)
    ...
    {
//    std::cout << "[JI]====================SetBeaconGeneration is called1================" << enable << std::endl;
...
{
//	std::cout << "[JI]============GetBeaconInterval is called================" << m_beaconInterval << std::endl;
...
{
//  std::cout << "[JI]============GetRawGroupInterval is called================" << m_rawGroupInterval << std::endl;
...
{
//	std::cout << "[JI]============GetSlotDurationCount is called================" << m_slotDurationCount << std::endl;
...
{
//	std::cout << "[JI]====================SetBeaconInterval is called================" << interval << std::endl;
...
{
//	std::cout << "[JI]====================SetRawGroupInterval is called================" << interval << std::endl;
...
{
//	std::cout << "[JI]====================SetTotalStaNum is called================" << num << std::endl;
...
{
//	std::cout << "[JI]====================SetSlotFormat is called================" << format << std::endl;
...
{
//	std::cout << "[JI]====================SetSlotDurationCount is called================" << count << std::endl;
...
{
//	std::cout << "[JI]====================SetSlotNum is called================" << count << std::endl;
...
{
//	std::cout << "[JI]====================StartBeaconing is called================" << std::endl;
...
//printf("[JI] ApWifiMac::ForwardDown is called\n");
...
//  std::cout << "[JI] AP ForwardDown hdr" << m_sendCount << " " << hdr << std::endl;
//  std::cout << "[JI] AP ForwardDown packet" << m_sendCount++ << " " << packet << std::endl;

//	std::cout << "[JI] AP ForwardDown hdr" << m_sendCount << " " << hdr << std::endl;
//	WifiMacTrailer fcs;
//	uint32_t fullPacketSize = hdr.GetSerializedSize () + packet->GetSize () + fcs.GetSerializedSize ();
//	std::cout << "[JI] STA ForwardDown packet" << m_sendCount++ << " Total length:" << fullPacketSize << std::endl;
/*
...
ApWifiMac::Enqueue (Ptr<const Packet> packet, Mac48Address to, Mac48Address from)
{
  ...
  if (to.IsBroadcast () || m_stationManager->IsAssociated (to))
    {
 //   printf("[JI] ApWifiMac::Enqueue is called\n");
...
//  std::cout << "[JI] AP SendProbeResp hdr" << m_sendCount << " " << hdr << std::endl;
//  std::cout << "[JI] AP SendProbeResp packet" << m_sendCount++ << " " << packet << std::endl;

//  std::cout << "[JI] AP SendProbeResp hdr" << m_sendCount << " " << hdr << std::endl;
//  WifiMacTrailer fcs;
//  uint32_t fullPacketSize = hdr.GetSerializedSize () + packet->GetSize () + fcs.GetSerializedSize ();
//  std::cout << "[JI] AP SendProbeResp packet" << m_sendCount++ << " Total length:" << fullPacketSize << std::endl;
/*
  	int size = packet->GetSize();
	printf("packet Size :%d " ,size);
	uint8_t buffer[1000] = {0,};
...
//  std::cout << "[JI] AP SendAssocResp hdr" << m_sendCount << " " << hdr << std::endl;
//  WifiMacTrailer fcs;
 // uint32_t fullPacketSize = hdr.GetSerializedSize () + packet->GetSize () + fcs.GetSerializedSize ();
 // std::cout << "[JI] AP SendAssocResp packet" << m_sendCount++ << " Total length:" << fullPacketSize << std::endl;
/*
  int size = packet->GetSize();
  printf("packet Size :%d " ,size);
  uint8_t buffer[1000] = {0,};
...
{/*
	 m_nowBeaconTime = clock();

	  printf("[JI] === SendOneBeacon is called === Time : %f DiffTime : %f STASendDataPacketCount : %d STASendAllPacketCount : %d APReceivedPacketCount : %d ReceiveRatio %f\n", 
	  	(float)(m_nowBeaconTime) / (float)CLOCKS_PER_SEC, (float)(m_nowBeaconTime - m_prevBeaconTime) / (float)CLOCKS_PER_SEC, g_dataPacketCount, g_dataPacketCount+ g_allPacketCount, g_ap_receivePacketCount, (float)g_ap_receivePacketCount / (float)g_dataPacketCount * (float)100.0);
	  
	  g_dataPacketCount = 0;
	  g_allPacketCount = 0;
	  g_ap_receivePacketCount = 0;

  	  m_prevBeaconTime = m_nowBeaconTime;
	  ...
	  m_beaconEvent = Simulator::Schedule (Seconds (0.1), &ApWifiMac::SendOneBeacon, this);
*/
    ...
    if (m_s1gSupported)
     {
      ...
      //raw.SetSlotCrossBoundary ((uint32_t)10);
//	  std::cout << "[JI]====================SendOneBeacon is called================" << m_slotCrossBoundary << std::endl;
	...
	// new feature
	 // JI
	uint8_t noSentNodeCount = 0;
	 uint8_t g_maxCount = 0;
	 uint8_t maxCountNode = 0;
	
	for(int i = 0; i < m_totalStaNum; i++)
	{
		if(g_bIsSent[i] == true)
		{
			g_noSentCount[i]++;
			noSentNodeCount++;
			if( g_maxCount < g_noSentCount[i])
			{
			   g_maxCount = g_noSentCount[i];
			   maxCountNode = i+1;
			}
		}
//		printf("[JI_LOG] noSentCount aid : %d(%d)\n", i+1, g_noSentCount[i]);
	}

	  //g_threshold = g_maxCount;	// god mode

	  if(g_traffic == 0)
	  	g_threshold = 0;
	  else if(g_traffic >= g_prevTraffic)
	  {
	  	//g_threshold++;
...
//	  g_threshold = 1;	// god mode
	 
	  // JI
	  raw.SetReservedInfo(g_threshold);

      m_rps.SetRawAssignment(raw);
      beacon.SetRPS (m_rps);

	  m_nowBeaconTime = Simulator::Now ();
	  g_nowBeaconTime = m_nowBeaconTime;

	  printf("[JI_LOG] === SendOneBeacon is called === aid_start : %d aid_end : %d g_contention : %d g_suceeseed : %d g_threshold : %d g_prevTraffic : %d nTraffic : %d noSentNodeCount : %d maxNode %d g_maxCount %d ", 
			  aid_start, aid_end, g_contentionCount, g_receviedCount, g_threshold, g_prevTraffic, g_traffic, noSentNodeCount, maxCountNode, g_maxCount);

	  std::cout << "Time : " << m_nowBeaconTime << " DiffTime : " << m_nowBeaconTime - m_prevBeaconTime << " STASendDataPacketCount : " << g_dataPacketCount << " STASendAllPacketCount : " << g_dataPacketCount+ g_allPacketCount << " APReceivedPacketCount : " << g_ap_receivePacketCount << " ReceiveRatio : " << (float)g_ap_receivePacketCount / (float)g_dataPacketCount * (float)100.0 << std::endl;


	printf("[STATE]\tg_threshold\t%d\t%d\t%d\n", g_threshold, g_traffic, g_bytePerNode);


	g_prevTraffic = g_traffic;
	g_traffic = 0;
...
//	  std::cout << "[JI] AP SendOneBeacon hdr" << m_sendCount << " " << hdr << std::endl;
//  	  std::cout << "[JI] AP SendOneBeacon packet" << m_sendCount++ << " " << packet << std::endl;

	  
//	  std::cout << "[JI] AP SendOneBeacon hdr" << m_sendCount << " " << hdr << std::endl;
//	  WifiMacTrailer fcs;
//	  uint32_t fullPacketSize = hdr.GetSerializedSize () + packet->GetSize () + fcs.GetSerializedSize ();
//	  std::cout << "[JI] AP SendOneBeacon packet" << m_sendCount++ << " Total length:" << fullPacketSize << std::endl;
/*
	  int size = packet->GetSize();
	  printf("packet Size :%d " ,size);
	  uint8_t buffer[1000] = {0,};
...
//	std::cout << "[JI] SendOneBeacon is rescheduling interval : " << m_beaconInterval << std::endl;
   m_beaconEvent = Simulator::Schedule (m_beaconInterval, &ApWifiMac::SendOneBeacon, this);
}
...
{
//  std::cout << "[JI] AP Receive hdr" << m_receiveCount << " " << hdr << std::endl;
//  std::cout << "[JI] AP Receive packet" << m_receiveCount++ << " " << packet << std::endl;

//  std::cout << "[JI] AP Receive hdr" << m_receiveCount << " " << *hdr << std::endl;
//  WifiMacTrailer fcs;
//  uint32_t fullPacketSize = hdr->GetSerializedSize () + packet->GetSize () + fcs.GetSerializedSize ();
//  std::cout << "[JI] AP Receive packet" << m_receiveCount++ << " Total length:" << fullPacketSize << std::endl;

  g_ap_receivePacketCount++;
/*
  int size = packet->GetSize();
  printf("packet Size :%d " ,size);
  uint8_t buffer[1000] = {0,};
          ...
          && m_stationManager->IsAssociated (from))
        {
          ...
          if (to == GetAddress ())
                   ...
                   || m_stationManager->IsAssociated (to))
            {
              ...
              if (hdr->IsQosData ())
                {
//                printf("[JI] ApWifiMac::Receive IsQosData is called\n");
                  ForwardDown (packet, from, to, hdr->GetQosTid ());
                }
              else
                {
 //               printf("[JI] ApWifiMac::Receive is called\n");
                  ForwardDown (packet, from, to);
                }
...
//			printf("[JI] ApWifiMac::DeaggregateAmsduAndForward is called\n");
...
{
//  printf("[JI] ApWifiMac::DoInitialize is called\n");
  ...
  if (m_enableBeaconGeneration)
    {
      if (m_enableBeaconJitter)
        {
 //       printf("[JI] ApWifiMac::DoInitialize is called1\n");
        ...
        {
//        printf("[JI] ApWifiMac::DoInitialize is called2\n");
